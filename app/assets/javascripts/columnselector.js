/* Column Selector/Responsive table widget (beta) for TableSorter 12/16/2013 (v2.14.6)
...
tsColSel = ts.columnSelector = {
	...
	setupSelector: function(table, c, wo) {
		...
		c.$table.children('thead').find('tr:first th', table).each(function() {
			...
			// set default state
			colSel.states[colId] = saved && saved.length >= colId ? saved[colId] : wo.columnSelector_columns[colId] ? wo.columnSelector_columns[colId] : true;
